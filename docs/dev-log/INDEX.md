...
- **alertas-clinicos-langgraph-pcdt** — agent:cursor — Pipeline multi-nó `clinical_alert_graph` (duas recuperações Chroma/rerank, interpretação rápida, assess com LLM opcional `MEDICO_CLINICAL_ALERTS_USE_LLM`); gatilhos em admissão, readmissão, vitais e exames (`patients.py`); `dedupe_key` em `alerts` + migração `20260525_1800`; audit JSONL `avaliacao_alerta_clinico_pcdt`; `async_client` com `lifespan` para testes ASGI exporem `app.state`. — Revisão: `ef44bd7` feat(rag): reescreve pergunta no primeiro turno
- **relatorio-secao-alertas-clinicos** — agent:cursor — `docs/relatorio de implementação.md`: pipeline `clinical_alert_graph` + secção **Auditoria em JSON** (`audit_clinical_*`, enums, pytest off, RAG técnico, `avaliacao_alerta_clinico_pcdt`); bullet “Alertas” não técnico. — Revisão: `085ccb7` Merge branch 'main' into alertas
