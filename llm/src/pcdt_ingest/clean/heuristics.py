...
_ADMIN_LINE_RE = re.compile(
    ...
    r"SECRETARIA DE VIGIL[ÂA]NCIA\b|"
    r"DEPARTAMENTO DE DOEN[ÇC]AS\b|"
    r"DI[ÁA]RIO OFICIAL\b"
    r")",
    re.IGNORECASE,
)
_FIGURE_CAPTION_RE = re.compile(
    r"^\s{0,3}#{0,3}\s*\*{0,2}\s*Figura\s+(?:\d+|[A-Z]|[IVXLCDM]+)[.)]?\b",
    re.IGNORECASE,
)
_TABLE_CAPTION_RE = re.compile(
    r"^\s{0,3}#{0,3}\s*\*{0,2}\s*Quadro\s+(?:\d+|[A-Z])[.)]?\b",
    re.IGNORECASE,
)
...
_PCDT_TITLE_LINE_RE = re.compile(
    r"^\s{0,3}\*{0,2}(?:ANEXO\s+)?"
    r"PROTOCOLO CL[IÍ]NICO E DIRETRIZES TERAP[EÊ]UTICAS(?:\s+D(?:A|AS|E|O|OS)\s+.+)?\*{0,2}\s*$",
    re.IGNORECASE,
)
_FORM_START_RE = re.compile(
    ...
    r"Documento de identifica[cç][aã]o do respons[aá]vel legal|"
    r"Assinatura do paciente|"
    r"Assinatura e carimbo do m[eé]dico|"
    r"M[eé]dico Respons[aá]vel:|"
    r"^\s*[-*]?\s*\(\s*\).*(?:\(\s*\).*)+|"
    r"Eu,\s*_{5,}.*declaro)",
    re.IGNORECASE,
)
_SEARCH_STRATEGY_RE = re.compile(
    r"estrat[eé]gias?\s+de\s+busca|"
    r"buscas?\s+sobre\s+interven[cç][oõ]es|"
    r"resultados encontrados|bases de dados|"
    r"\bLocalizados\b|\bSelecionados\b|"
    r"\bPICO\b|Medline|PubMed|\[Mesh\]|\[MeSH Terms\]|\[PDAT\]",
    re.IGNORECASE,
)
...
def is_search_strategy_caption_line(line: str) -> bool:
    stripped = line.strip()
    return bool(_SEARCH_STRATEGY_CAPTION_RE.search(stripped) or _SEARCH_STRATEGY_RE.search(stripped))


def is_lone_annex_line(line: str) -> bool:
    return bool(_LONE_ANNEX_RE.fullmatch(line.strip()))
...
def is_broken_table_header_line(line: str) -> bool:
    return bool(is_table_line(line) and _BROKEN_BOLD_WORD_RE.search(line))
...
def is_pcdt_title_line(line: str) -> bool:
    return bool(_PCDT_TITLE_LINE_RE.fullmatch(line.strip()))
...
def is_malformed_table_block(lines: Sequence[str]) -> bool:
    ...
    for line in table_lines:
        cells.extend(split_table_cells(line))
    if not cells:
        return False
    ...
    broken_bold_headers = sum(1 for line in table_lines if _BROKEN_BOLD_WORD_RE.search(line))
    ...
    return (
        (avg_cells >= 5 and empty_ratio >= 0.35)
        or fragment_ratio >= 0.30
        or (broken_bold_headers >= 2 and len(table_lines) >= 3)
        or repeated_header_rows >= 2
    )
