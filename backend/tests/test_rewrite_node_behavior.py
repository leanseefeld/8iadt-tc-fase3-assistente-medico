...
@pytest.mark.asyncio
async def test_rewrite_first_turn_uses_llm_rewrite(monkeypatch):
    async def _resolve(*, state, query, settings, trace=None):
        _ = (state, query, settings, trace)
        return RetrievalQueryResolution(
            retrieval_query="critérios de inclusão síndrome de guillain-barré",
            reasoning_steps=["Busca: pergunta reescrita para recuperação."],
            used_history=False,
            llm_used=True,
        )
    ...
    assert out["retrieval_query"] == "critérios de inclusão síndrome de guillain-barré"
